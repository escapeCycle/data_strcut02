...
public class Proxy {
    ...
    public static Object newProxyInstance(Class inf, InvocationHandler handler) throws Exception {
        ...
        JavaFile javaFile = JavaFile.builder("com.xunhuan.java.designmodel.dynamicproxy.demo", typeSpecBuilder.build()).build();
        // 为了看的更清楚，我将源码文件生成到桌面
        String sourcePath = "D:/";
        javaFile.writeTo(new File(sourcePath));

        // 编译
        JavaCompiler.compile(new File(sourcePath + "/com/xunhuan/java/designmodel/dynamicproxy/demo/TimeProxy.java"));

        // 使用反射load到内存
        URL[] urls = new URL[] {new URL("file:" + sourcePath)};
        URLClassLoader classLoader = new URLClassLoader(urls);
        Class clazz = classLoader.loadClass("com.xunhuan.java.designmodel.dynamicproxy.demo.TimeProxy");
